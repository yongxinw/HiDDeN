...
def save_images(original_images, watermarked_images, epoch, folder, resize_to=None):
    ...
    torchvision.utils.save_image(
        stacked_images, filename, original_images.shape[0], normalize=False
    )
...
def save_checkpoint(
    model: Hidden, experiment_name: str, epoch: int, checkpoint_folder: str
):
...
def get_data_loaders(
        ...
        "train": transforms.Compose(
            [
                transforms.RandomCrop(
                    (hidden_config.H, hidden_config.W), pad_if_needed=True
                ),
                transforms.ToTensor(),
    ...
    train_images = datasets.ImageFolder(
        train_options.train_folder, data_transforms["train"]
    )

    validation_images = datasets.ImageFolder(
        train_options.validation_folder, data_transforms["test"]
    )
...
def log_print_helper(losses_accu, log_or_print_func):
    ...
    for loss_name, loss_value in losses_accu.items():
        log_or_print_func(
            loss_name.ljust(max_len + 4) + "{:.4f}".format(loss_value.avg)
        )
...
def write_losses(file_name, losses_accu, epoch, duration):
    with open(file_name, "a", newline="") as csvfile:
        ...
        row_to_write = (
            [epoch]
            + ["{:.4f}".format(loss_avg.avg) for loss_avg in losses_accu.values()]
            + ["{:.0f}".format(duration)]
        )
        writer.writerow(row_to_write)
