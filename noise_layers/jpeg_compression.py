        ...
        for k_x in range(size_x):
            for n_y in range(size_y):
                for n_x in range(size_x):
                    filters[k_y * tile_size_x + k_x, n_y, n_x] = dct_or_idct_fun(
                        n_y, k_y, size_y
...
def get_jpeg_yuv_filter_mask(image_shape: tuple, window_size: int, keep_count: int):
    ...
    return np.tile(
        mask,
        (
            int(np.ceil(image_shape[0] / window_size)),
            int(np.ceil(image_shape[1] / window_size)),
        ),
    )[0 : image_shape[0], 0 : image_shape[1]]


def dct_coeff(n, k, N):
    return np.cos(np.pi / N * (n + 1.0 / 2.0) * k)


def idct_coeff(n, k, N):
    return (int(0 == n) * (-1 / 2) + np.cos(np.pi / N * (k + 1.0 / 2.0) * n)) * np.sqrt(
        1 / (2.0 * N)
    )
...
def yuv2rgb(image_yuv, image_rgb_out):
    ...
    image_rgb_out[:, 2, :, :] = (
        image_yuv[:, 0, :, :].clone() + 2.03211 * image_yuv[:, 1, :, :].clone()
    )

    # return image_rgb_out.clone()
...
class JpegCompression(nn.Module):
    def __init__(self, device, yuv_keep_weights=(25, 9, 9)):
        ...
        self.dct_conv_weights = torch.tensor(
            gen_filters(8, 8, dct_coeff), dtype=torch.float32
        ).cuda()
        self.dct_conv_weights.unsqueeze_(1)
        self.idct_conv_weights = torch.tensor(
            gen_filters(8, 8, idct_coeff), dtype=torch.float32
        ).cuda()
        ...
        if self.jpeg_mask is None or requested_shape > self.jpeg_mask.shape[1:]:
            ...
            for channel, weights_to_keep in enumerate(self.yuv_keep_weighs):
                mask = torch.from_numpy(
                    get_jpeg_yuv_filter_mask(requested_shape, 8, weights_to_keep)
                )
    ...
    def apply_conv(self, image, filter_type: str):
        ...
        for channel in range(image.shape[1]):
            ...
            image_conv = image_conv.view(
                image_conv.shape[0], image_conv.shape[1], image_conv.shape[2], 8, 8
            )
            image_conv = image_conv.permute(0, 1, 3, 2, 4)
            image_conv = image_conv.contiguous().view(
                image_conv.shape[0],
    ...
    def forward(self, noised_and_cover):
        ...
        # un-pad
        noised_and_cover[0] = image_ret_padded[
            :,
            :,
